...
mu_X <- (((0.00 <= X_all[, 1]) & (X_all[, 1] < 0.25)) *
  (-7.5) +
...
X_train_u <- X_all[train_inds, ]
X_test_u <- X_all[test_inds, ]
Z_train_u <- Z_all[train_inds]
Z_test_u <- Z_all[test_inds]
pi_train_u <- pi_X[train_inds]
pi_test_u <- pi_X[test_inds]
y_train_u <- y_all[train_inds]
y_test_u <- y_all[test_inds]
...
X_train_mv <- X_all_mv[train_inds_mv, ]
X_test_mv <- X_all_mv[test_inds_mv, ]
Z_train_mv <- Z_mv[train_inds_mv, ]
Z_test_mv <- Z_mv[test_inds_mv, ]
pi_train_mv <- pi_mv[train_inds_mv, ]
pi_test_mv <- pi_mv[test_inds_mv, ]
y_train_mv <- y_mv[train_inds_mv]
y_test_mv <- y_mv[test_inds_mv]
...
run_once_multivariate <- function(run_cpp, seed = -1) {
  ...
  preds <- predict(
    ...
    X = X_test_mv,
    Z = Z_test_mv,
    propensity = pi_test_mv,
    run_cpp = run_cpp
  )
  elapsed_predict <- (proc.time() - t1)[["elapsed"]]

  tau_0_shape <- if (!is.null(m$tau_0_samples)) dim(m$tau_0_samples) else NULL
  tau_0_mean_0 <- if (!is.null(m$tau_0_samples)) {
    mean(m$tau_0_samples[1, ])
  } else {
    NA_real_
  }
  tau_0_mean_1 <- if (!is.null(m$tau_0_samples)) {
    mean(m$tau_0_samples[2, ])
  } else {
    NA_real_
  }
...
cat(sprintf(
  "tau_0_samples shape  cpp=%s  R=%s\n",
  if (is.null(shape_cpp)) {
    "NULL"
  } else {
    paste0("[", paste(shape_cpp, collapse = ","), "]")
  },
  if (is.null(shape_r)) {
    "NULL"
  } else {
    paste0("[", paste(shape_r, collapse = ","), "]")
  }
))
cat(sprintf(
...
cat(sprintf(
  "tau_0_samples shape  cpp=%s  R=%s\n",
  if (is.null(shape_cpp_mv)) {
    "NULL"
  } else {
    paste0("[", paste(shape_cpp_mv, collapse = ","), "]")
  },
  if (is.null(shape_r_mv)) {
    "NULL"
  } else {
    paste0("[", paste(shape_r_mv, collapse = ","), "]")
  }
))
cat(sprintf(
...
for (i in seq_len(nrow(res_mv))) {
  cat(sprintf(
    ...
    res_mv$sampler[i],
    res_mv$elapsed_mean[i],
    res_mv$elapsed_sd[i],
    res_mv$tau_0_mean_0[i],
    res_mv$tau_0_mean_1[i],
    res_mv$rmse_y[i],
    res_mv$rmse_f[i],
    res_mv$rmse_tau1[i],
    res_mv$rmse_tau2[i]
