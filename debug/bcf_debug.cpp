 ...
 *
 * TODO: Replace the stub include with <stochtree/bcf_sampler.h> once
 *       Task 2.2 (src/bcf_sampler.cpp) is complete.
 */

// TODO: Replace with #include <stochtree/bcf_sampler.h> once Task 2.2 is complete.
#include "bcf_sampler_stub.h"
...
static void report_bcf(const StochTree::BCFSamples& samples,
  ...
  const int n_test      = samples.n_test;
  ...
  for (int i = 0; i < n_test; i++) {
    ...
    for (int j = 0; j < num_samples; j++) {
      ...
      mu_hat  += samples.mu_hat_test[k]  / num_samples;
      tau_hat += samples.tau_hat_test[k] / num_samples;
...
static void run_scenario_0(int n, int n_test, int p,
  ...
  data.z_train       = train.z.data();
  data.n_train       = n;
  data.p_x           = p;
  data.treatment_dim = 1;
  data.X_test        = test.X.data();
  data.z_test        = test.z.data();
  data.n_test        = n_test;
  ...
  StochTree::BCFSamplerFit(&samples, config, data);
...
static void run_scenario_1(int n, int n_test, int p,
  ...
  StochTree::BCFSamples samples;
  StochTree::BCFSamplerFit(&samples, config, data);
  // Predictions are on latent scale; compare tau to true CATE and y to latent W.
  report_bcf(samples, test.mu_true, test.tau_true, test.latent_outcome,
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if ((arg == "--scenario" || arg == "--n" || arg == "--n_test" || arg == "--p" ||
         ...
         arg == "--num_mcmc" || arg == "--seed") && i + 1 < argc) {
      int val = std::stoi(argv[++i]);
      if      (arg == "--scenario")      scenario      = val;
      else if (arg == "--n")             n             = val;
      else if (arg == "--n_test")        n_test        = val;
      else if (arg == "--p")             p             = val;
      else if (arg == "--num_trees_mu")  num_trees_mu  = val;
      else if (arg == "--num_trees_tau") num_trees_tau = val;
      else if (arg == "--num_gfr")       num_gfr       = val;
      else if (arg == "--num_mcmc")      num_mcmc      = val;
      else if (arg == "--seed")          seed          = val;
    } else {
  ...
  switch (scenario) {
    case 0: run_scenario_0(n, n_test, p, num_trees_mu, num_trees_tau, num_gfr, num_mcmc, seed); break;
    case 1: run_scenario_1(n, n_test, p, num_trees_mu, num_trees_tau, num_gfr, num_mcmc, seed); break;
    default:
