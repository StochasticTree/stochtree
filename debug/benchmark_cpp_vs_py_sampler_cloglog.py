...
f(X) is a sum of smooth sinusoidal terms across two covariates, keeping
probabilities in the moderate range [0.25, 0.75] for stable mixing.
GFR is disabled (num_gfr=0) since it interacts poorly with cloglog binary.
...
# in [~0.25, ~0.75], avoiding extreme probabilities that inflate GFR depth.
f_X = (
    0.6 * np.sin(2 * np.pi * X[:, 0])
    + 0.4 * np.cos(2 * np.pi * X[:, 1])
    - 0.5
)
