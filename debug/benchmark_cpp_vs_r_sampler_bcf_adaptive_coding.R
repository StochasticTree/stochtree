...
Z_train <- Z_all[train_inds]
Z_test <- Z_all[test_inds]
...
cat(sprintf(
  "True b_0=%.1f  b_1=%.1f  CATE=(b1-b0)*tau_forest(X)\n",
  TRUE_B_0,
  TRUE_B_1
))
cat(strrep("=", 60), "\n")
...
cat("\n--- Results ---\n")
cat(sprintf(
  "b_0_samples length  cpp=%d  R=%d\n",
  results_cpp[[1]]$b0_length,
  results_r[[1]]$b0_length
))
cat(sprintf(
  "b_1_samples length  cpp=%d  R=%d\n",
  results_cpp[[1]]$b1_length,
  results_r[[1]]$b1_length
))
cat("\n")
cat(sprintf(
...
for (i in seq_len(nrow(res))) {
  cat(sprintf(
    ...
    res$b0_mean[i],
    res$b1_mean[i],
    res$max_decomp_err_train[i],
    res$max_decomp_err_test[i],
    res$rmse_y[i],
    res$rmse_f[i],
    res$rmse_tau[i]
  ))
