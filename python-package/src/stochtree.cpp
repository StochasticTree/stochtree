...
class ForestDatasetCpp {
 public:
  ...
  void AddCovariates(py::array covariate_matrix, data_size_t num_row, int num_col, bool row_major) {
  ...
  void AddBasis(py::array basis_matrix, data_size_t num_row, int num_col, bool row_major) {
    // Extract pointer to contiguous block of memory
  ...
  void AddVarianceWeights(py::array weight_vector, data_size_t num_row) {
    // Extract pointer to contiguous block of memory
...
class ResidualCpp {
 public:
  ResidualCpp(py::array residual_array, data_size_t num_row) {
    // Extract pointer to contiguous block of memory
  ...
  py::array_t<double> Predict(ForestDatasetCpp& dataset) {
    ...
    for (size_t i = 0; i < n; i++) {
      for (int j = 0; j < num_samples; j++) {
        ...
        ptr[i*num_samples + j] = output_raw[n*j + i];
      }
    }
