...
#
# Includes three command line arguments:
...
#
# For CRAN preparation, please run:
# -----------------------------------------------------
...
r_src_files <- r_src_files[
  !(r_src_files %in% c("src/Makevars", "src/Makevars.win"))
]
cat(r_src_files)
pkg_core_files <- c(
  ".Rbuildignore",
...
if (all(file.exists(pkg_core_files))) {
  ...
  if (n_removed > 0) {
    cat(sprintf(
      "Removed %d previously vendored files from temporary CRAN directory (%s)\n",
      n_removed,
      cran_dir
    ))
  }
...
makevars_lines[grep(
  ...
  makevars_lines,
  fixed = T
)] <- "    -I$(PKGROOT)/src/include \\"
makevars_lines <- makevars_lines[
  -c(
...
makevars_win_lines[grep(
  "    -I$(PKGROOT)/include \\",
  makevars_win_lines,
  fixed = T
)] <- "    -I$(PKGROOT)/src/include \\"
makevars_win_lines <- makevars_win_lines[
  -c(
...
if (!include_tests) {
  ...
  description_lines <- description_lines[
    -c(suggestion_lines, testthat_config_line)
  ]
  writeLines(description_lines, cran_description)
}
...
fast_double_parser_src <- file.path(
  "deps/fast_double_parser/include/fast_double_parser.h"
)
fast_double_parser_dst <- file.path(
  cran_dir,
  "src/include/fast_double_parser.h"
)
if (file.exists(fast_double_parser_src)) {
  file_removed <- suppressWarnings(sum(file.remove(fast_double_parser_dst)) > 0)
  if (file_removed) {
    cat(sprintf(
      "Removed previously vendored fast_double_parser.h file from src/include\n"
    ))
  }
...
if (all(file.exists(header_files_to_vendor_src))) {
  ...
  if (n_removed > 0) {
    cat(sprintf(
      "Removed %d previously vendored files from src/include\n",
      n_removed
    ))
  }
...
fmt_header_filenames_src <- list.files(
  fmt_header_subfolder_src,
  pattern = "\\.(h)$",
  recursive = TRUE
)
fmt_header_files_to_vendor_src <- file.path(
  fmt_header_subfolder_src,
  fmt_header_filenames_src
)
# Destination files
fmt_header_subfolder_dst <- "src/include/fmt"
fmt_header_files_to_vendor_dst <- file.path(
  cran_dir,
  fmt_header_subfolder_dst,
  basename(fmt_header_filenames_src)
)

if (all(file.exists(fmt_header_files_to_vendor_src))) {
  n_removed <- suppressWarnings(sum(file.remove(
    fmt_header_files_to_vendor_dst
  )))
  if (n_removed > 0) {
    cat(sprintf(
      "Removed %d previously vendored files from src/include/fmt\n",
      n_removed
    ))
  }
  ...
  if (
    all(file.copy(
      fmt_header_files_to_vendor_src,
      fmt_header_files_to_vendor_dst
    ))
  ) {
    cat(
...
for (eigen_mod in eigen_modules) {
  ...
  eigen_module_source_src <- list.files(
    eigen_module_subfolder_src,
    recursive = TRUE,
    full.names = TRUE
  )
  if (eigen_mod %in% c("misc", "plugins")) {
    eigen_files_to_vendor_src <- c(
      eigen_files_to_vendor_src,
      eigen_module_source_src
    )
  } else {
    eigen_module_header_src <- file.path("deps/eigen/Eigen", eigen_mod)
    eigen_files_to_vendor_src <- c(
      eigen_files_to_vendor_src,
      eigen_module_header_src,
      eigen_module_source_src
    )
  }

  # Destination files
  eigen_module_source_dst <- gsub(
    "deps/eigen/Eigen",
    paste0(cran_dir, "/src/include/Eigen"),
    eigen_module_source_src
  )
  if (eigen_mod %in% c("misc", "plugins")) {
    eigen_files_to_vendor_dst <- c(
      eigen_files_to_vendor_dst,
      eigen_module_source_dst
    )
  } else {
    eigen_module_header_dst <- file.path(
      cran_dir,
      "src/include/Eigen",
      eigen_mod
    )
...
if (all(file.exists(eigen_files_to_vendor_src))) {
  ...
  if (n_removed > 0) {
    cat(sprintf(
      "Removed %d previously vendored files from src/include/Eigen\n",
      n_removed
    ))
  }
...
for (i in 1:length(eigen_suppress_warnings_lines)) {
  ...
  eigen_suppress_warnings_lines[i] <- gsub(
    "^.*#pragma clang diagnostic.*$",
    "",
    eigen_suppress_warnings_lines[i]
  )
  eigen_suppress_warnings_lines[i] <- gsub(
