...
BARTPredictionResult predict_bart_model(BARTData& data, BARTSamples& samples, BARTPredictionMetadata& metadata) {
  ...
  const int num_samples = metadata.num_samples;
  const int num_obs = metadata.num_obs;

  // Key model components
  const bool has_mean_forest = samples.mean_forests != nullptr;
  const bool has_variance_forest = samples.variance_forests != nullptr;
  const bool has_rfx = metadata.has_rfx;
  const bool rfx_custom = metadata.rfx_model_spec == BARTRFXModelSpec::Custom;
  ...
  if (metadata.pred_scale == PredScale::kClass && metadata.pred_type == PredType::kMean) {
  ...
  if (metadata.pred_type == PredType::kPosterior) {
    num_samples_output = num_samples;
  }
  bool need_mean = has_mean_forest && (metadata.pred_terms.y_hat || metadata.pred_terms.mean_forest);
  bool need_rfx = has_rfx && (metadata.pred_terms.y_hat || metadata.pred_terms.random_effects);
  bool need_variance_forest = has_variance_forest && metadata.pred_terms.variance_forest;

  // Resize any output vectors to be returned to users
  const bool probability_scale = metadata.pred_scale == PredScale::kProbability;
  const bool class_scale = metadata.pred_scale == PredScale::kClass;
  const bool ordinal_cloglog_prob_scale = probability_scale && metadata.link_function == LinkFunction::Cloglog && metadata.outcome_type == OutcomeType::Ordinal;
  if (metadata.pred_terms.y_hat) output.y_hat.resize(num_obs * (ordinal_cloglog_prob_scale ? metadata.cloglog_num_classes : 1) * num_samples_output);
  if (metadata.pred_terms.mean_forest) output.mean_forest_predictions.resize(num_obs * (ordinal_cloglog_prob_scale ? metadata.cloglog_num_classes : 1) * num_samples_output);
  if (metadata.pred_terms.variance_forest) output.variance_forest_predictions.resize(num_obs * num_samples_output);
  if (metadata.pred_terms.random_effects) output.rfx_predictions.resize(num_obs * num_samples_output);
  ...
  if (need_mean) {
    ...
    mean_forest = samples.mean_forests->Predict(forest_dataset);
  }
  ...
  if (need_rfx) {
    ...
    samples.rfx_container->Predict(rfx_dataset, *samples.rfx_label_mapper, rfx);
  }

  if (need_variance_forest) {
    variance_forest = samples.variance_forests->Predict(forest_dataset);
  }
  if (metadata.pred_terms.y_hat) {
  ...
  // Scale the outputs
  if (metadata.pred_terms.mean_forest) {
    location_scale_adjust_predictions(mean_forest, samples.y_bar, samples.y_std);
  }
  if (metadata.pred_terms.random_effects) {
    location_scale_adjust_predictions(rfx, 0.0, samples.y_std);
  }
  if (metadata.pred_terms.y_hat) {
    location_scale_adjust_predictions(y_hat, samples.y_bar, samples.y_std);
  }
  if (need_variance_forest) {
    location_scale_adjust_predictions(variance_forest, 0.0, samples.y_std * samples.y_std);
  }

  // Transform if necessary (e.g. for probit models)
  if (metadata.link_function == LinkFunction::Probit) {
    if (metadata.pred_terms.mean_forest && probability_scale) {
      probability_transform_probit_predictions(mean_forest);
    }
    if (metadata.pred_terms.random_effects && probability_scale) {
      probability_transform_probit_predictions(rfx);
    }
    if (metadata.pred_terms.y_hat && (probability_scale || class_scale)) {
      probability_transform_probit_predictions(y_hat);
      if (class_scale) {
        class_transform_binary_outcome_predictions(y_hat);
      }
    }
  } else if (metadata.link_function == LinkFunction::Cloglog) {
    if (metadata.outcome_type == OutcomeType::Binary) {
      if (metadata.pred_terms.mean_forest && probability_scale) {
    ...
    } else if (metadata.outcome_type == OutcomeType::Ordinal) {
      if (metadata.pred_terms.mean_forest && probability_scale) {
        std::vector<double> mean_forest_prob(num_obs * num_samples * metadata.cloglog_num_classes);
        probability_transform_ordinal_cloglog_predictions(mean_forest, mean_forest_prob, samples.cloglog_cutpoint_samples.data(), num_obs, metadata.cloglog_num_classes, num_samples);
        mean_forest = std::move(mean_forest_prob);
      }
      if (metadata.pred_terms.y_hat && (probability_scale || class_scale)) {
        std::vector<double> y_hat_prob(num_obs * num_samples * metadata.cloglog_num_classes);
        probability_transform_ordinal_cloglog_predictions(y_hat, y_hat_prob, samples.cloglog_cutpoint_samples.data(), num_obs, metadata.cloglog_num_classes, num_samples);
        if (metadata.pred_scale == PredScale::kClass) {
          class_transform_multiclass_outcome_predictions(y_hat_prob, y_hat, num_obs, metadata.cloglog_num_classes, num_samples);
        } else {
  ...
  if (metadata.pred_terms.mean_forest) {
    if (metadata.pred_type == PredType::kMean) {
      if (metadata.pred_scale == PredScale::kProbability && metadata.outcome_type == OutcomeType::Ordinal && metadata.link_function == LinkFunction::Cloglog) {
        average_col_major_3d(mean_forest, output.mean_forest_predictions, /*dim1=*/num_obs, /*dim2=*/metadata.cloglog_num_classes, /*dim3=*/num_samples, /*dim_average=*/2);
      } else {
  ...
  if (metadata.pred_terms.y_hat) {
    if (metadata.pred_type == PredType::kMean) {
      if (metadata.pred_scale == PredScale::kProbability && metadata.outcome_type == OutcomeType::Ordinal && metadata.link_function == LinkFunction::Cloglog) {
        average_col_major_3d(y_hat, output.y_hat, /*dim1=*/num_obs, /*dim2=*/metadata.cloglog_num_classes, /*dim3=*/num_samples, /*dim_average=*/2);
      } else {
...
BCFPredictionResult predict_bcf_model(BCFData& data, BCFSamples& samples, BCFPredictionMetadata& metadata) {
  ...
  const int num_samples = metadata.num_samples;
  const int num_obs = metadata.num_obs;
  const int num_treatment = metadata.treatment_dim;

  // Key model components
  const bool has_mu_forest = samples.mu_forests != nullptr;
  const bool has_tau_forest = samples.tau_forests != nullptr;
  const bool has_variance_forest = samples.variance_forests != nullptr;
  const bool has_rfx = samples.rfx_container != nullptr;
  const bool rfx_custom = metadata.rfx_model_spec == BCFRFXModelSpec::Custom;
  const bool rfx_intercept = metadata.rfx_model_spec == BCFRFXModelSpec::InterceptOnly || metadata.rfx_model_spec == BCFRFXModelSpec::InterceptPlusTreatment;
  const bool rfx_treatment = metadata.rfx_model_spec == BCFRFXModelSpec::InterceptPlusTreatment;
  ...
  bool need_tau_interm = metadata.pred_terms.y_hat || metadata.pred_terms.tau_x || metadata.pred_terms.cate;
  bool need_mu = metadata.pred_terms.y_hat || metadata.pred_terms.mu_x || metadata.pred_terms.prognostic_function || (metadata.adaptive_coding && need_tau_interm);
  bool need_tau = need_tau_interm || (metadata.adaptive_coding && need_mu);
  bool need_rfx = has_rfx && (metadata.pred_terms.y_hat || metadata.pred_terms.random_effects);
  bool need_rfx_intercept = has_rfx && rfx_intercept && metadata.pred_terms.prognostic_function;
  bool need_rfx_treatment = has_rfx && rfx_treatment && metadata.pred_terms.cate;
  bool need_variance_forest = has_variance_forest && metadata.pred_terms.conditional_variance;

  // Resize any output vectors to be returned to users
  if (metadata.pred_terms.y_hat) output.y_hat.resize(num_obs * num_samples_output);
  if (metadata.pred_terms.mu_x) output.mu_x.resize(num_obs * num_samples_output);
  if (metadata.pred_terms.tau_x) output.tau_x.resize(num_obs * num_treatment * num_samples_output);
  if (metadata.pred_terms.prognostic_function) output.prognostic_function.resize(num_obs * num_samples_output);
  if (metadata.pred_terms.cate) output.cate.resize(num_obs * num_treatment * num_samples_output);
  if (metadata.pred_terms.conditional_variance) output.conditional_variance.resize(num_obs * num_samples_output);
  if (metadata.pred_terms.random_effects) output.random_effects.resize(num_obs * num_samples_output);
  ...
  if (need_mu) {
    ...
    mu_x = samples.mu_forests->Predict(forest_dataset);
  }
  ...
  if (need_tau) {
    ...
    tau_x = samples.tau_forests->PredictRaw(forest_dataset, /*row_major=*/false);
    ...
    // tau_0_samples is stored in the ORIGINAL outcome scale (matching sigma2_global, the other
    // parametric terms, and the R main-branch convention), whereas tau_x here is standardized (from
    // PredictRaw). Divide tau_0 by y_std to add it in standardized space; the y_std scale step applied
    // later (location_scale_adjust_predictions) then scales the full CATE tau_0 + tau(x) to original.
    if (metadata.sample_tau_0) {
      for (int j = 0; j < num_samples; j++) {
        for (int k = 0; k < num_treatment; k++) {
          for (int i = 0; i < num_obs; i++) {
            const int idx = j * num_obs * num_treatment + k * num_obs + i;
            tau_x[idx] += samples.tau_0_samples[j * num_treatment + k] / metadata.y_std;
          }
        }
      }
    }
  ...
  if (need_rfx_intercept) {
    ...
    std::vector<double>& beta = samples.rfx_container->GetBeta();
    int num_components = samples.rfx_container->NumComponents();
    int num_groups = samples.rfx_container->NumGroups();
    for (int i = 0; i < num_obs; i++) {
      group_ind = samples.rfx_label_mapper->CategoryNumber(data.rfx_group_ids_test[i]);
      for (int j = 0; j < num_samples; j++) {
  ...
  if (need_rfx) {
    ...
    if (data.rfx_basis_test != nullptr) {
      rfx_dataset.AddBasis(data.rfx_basis_test, num_obs, data.rfx_basis_dim, /*row_major=*/false);
    } else if (metadata.rfx_model_spec == BCFRFXModelSpec::InterceptOnly) {
      std::vector<double> rfx_basis(data.n_test, 1.0);
      rfx_dataset.AddBasis(rfx_basis.data(), num_obs, 1, /*row_major=*/false);
    } else if (metadata.rfx_model_spec == BCFRFXModelSpec::InterceptPlusTreatment) {
  ...
  // Scale the outputs
  if (metadata.pred_terms.mu_x) {
    location_scale_adjust_predictions(mu_x, samples.y_bar, samples.y_std);
  }
  if (metadata.pred_terms.prognostic_function) {
    location_scale_adjust_predictions(prognostic_function, samples.y_bar, samples.y_std);
  }
  if (metadata.pred_terms.tau_x) {
    location_scale_adjust_predictions(tau_x, 0.0, samples.y_std);
  }
  if (metadata.pred_terms.cate) {
    location_scale_adjust_predictions(cate, 0.0, samples.y_std);
  }
  if (metadata.pred_terms.random_effects) {
    location_scale_adjust_predictions(rfx, 0.0, samples.y_std);
  }
  ...
  const bool probability_scale = metadata.pred_scale == PredScale::kProbability;
  const bool class_scale = metadata.pred_scale == PredScale::kClass;
  if (metadata.pred_terms.mu_x && probability_scale) {
    probability_transform_probit_predictions(mu_x);
  }
  if (metadata.pred_terms.prognostic_function && probability_scale) {
    probability_transform_probit_predictions(prognostic_function);
  }
  if (metadata.pred_terms.tau_x && probability_scale) {
    probability_transform_probit_predictions(tau_x);
  }
  if (metadata.pred_terms.cate && probability_scale) {
