...
void FeatureCutpointGrid::CalculateStrides(Dataset* dataset, SortedNodeSampleTracker* feature_node_sort_tracker, int32_t node_id, data_size_t node_begin, data_size_t node_end, int32_t feature_index) {
  ...
  FeatureType feature_type = dataset->GetFeatureType(feature_index);
  if (feature_type == FeatureType::kNumeric) {
    CalculateStridesNumeric(dataset, feature_node_sort_tracker, node_id, node_begin, node_end, feature_index);
  } else if (feature_type == FeatureType::kOrderedCategorical) {
    CalculateStridesOrderedCategorical(dataset, feature_node_sort_tracker, node_id, node_begin, node_end, feature_index);
  } else if (feature_type == FeatureType::kUnorderedCategorical) {
    CalculateStridesUnorderedCategorical(dataset, feature_node_sort_tracker, node_id, node_begin, node_end, feature_index);
  }
}

void FeatureCutpointGrid::CalculateStridesNumeric(Dataset* dataset, SortedNodeSampleTracker* feature_node_sort_tracker, int32_t node_id, data_size_t node_begin, data_size_t node_end, int32_t feature_index) {
  ...
  if (node_size <= cutpoint_grid_size_) {
    ...
    EnumerateNumericCutpointsDeduplication(dataset, feature_node_sort_tracker, node_id, node_begin, node_end, node_size, feature_index);
  } else {
    ...
    ScanNumericCutpoints(dataset, feature_node_sort_tracker, node_id, node_begin, node_end, node_size, feature_index);
  }
}

void FeatureCutpointGrid::CalculateStridesOrderedCategorical(Dataset* dataset, SortedNodeSampleTracker* feature_node_sort_tracker, int32_t node_id, data_size_t node_begin, data_size_t node_end, int32_t feature_index) {
  data_size_t node_size = node_end - node_begin;
  ...
  if (node_end - node_begin == 1) {
    ...
    single_value = dataset->CovariateValue(feature_node_sort_tracker->SortIndex(node_begin, feature_index), feature_index);
    cutpoint_values_.push_back(static_cast<std::uint32_t>(single_value));
    return;
  }

  // Edge case 2: single category
  std::uint32_t first_val = static_cast<std::uint32_t>(dataset->CovariateValue(feature_node_sort_tracker->SortIndex(node_begin, feature_index), feature_index));
  std::uint32_t last_val = static_cast<std::uint32_t>(dataset->CovariateValue(feature_node_sort_tracker->SortIndex(node_end - 1, feature_index), feature_index));
  ...
  for (data_size_t i = node_begin; i < node_end; i++){
    ...
    current_val = dataset->CovariateValue(current_sort_ind, feature_index);
    ...
    if (last_element) {
      ...
      next_val = dataset->CovariateValue(next_sort_ind, feature_index);
...
void FeatureCutpointGrid::CalculateStridesUnorderedCategorical(Dataset* dataset, SortedNodeSampleTracker* feature_node_sort_tracker, int32_t node_id, data_size_t node_begin, data_size_t node_end, int32_t feature_index) {
  ...
  for (data_size_t i = node_begin; i < node_end; i++){
    ...
    // Increment stride length and bin_sum
    stride_length += 1;
    bin_sum += dataset->OutcomeValue(current_sort_ind, 0);
...
void FeatureCutpointGrid::EnumerateNumericCutpointsDeduplication(Dataset* dataset, SortedNodeSampleTracker* feature_node_sort_tracker, int32_t node_id, data_size_t node_begin, data_size_t node_end, data_size_t node_size, int32_t feature_index) {
  // Edge case 1: single observation
  double single_value;
  if (node_end - node_begin == 1) {
    node_stride_begin_.push_back(node_begin);
  ...
  double first_val = dataset->CovariateValue(feature_node_sort_tracker->SortIndex(node_begin, feature_index), feature_index);
  double last_val = dataset->CovariateValue(feature_node_sort_tracker->SortIndex(node_end - 1, feature_index), feature_index);
...
void FeatureCutpointGrid::ScanNumericCutpoints(Dataset* dataset, SortedNodeSampleTracker* feature_node_sort_tracker, int32_t node_id, data_size_t node_begin, data_size_t node_end, data_size_t node_size, int32_t feature_index) {
  // Edge case 1: single observation
  double single_value;
  if (node_end - node_begin == 1) {
    node_stride_begin_.push_back(node_begin);
