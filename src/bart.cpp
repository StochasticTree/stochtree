...
BARTResult BARTDispatcher::CreateOutputObject(int num_trees, int output_dimension, bool is_leaf_constant) {
  return BARTResult(num_trees, output_dimension, is_leaf_constant);
}

void BARTDispatcher::AddDataset(double* covariates, data_size_t num_row, int num_col, bool is_row_major, bool train) {
  if (train) {
...
void BARTDispatcher::AddTrainOutcome(double* outcome, data_size_t num_row) {
  train_outcome_ = ColumnVector();
  train_outcome_.LoadData(outcome, num_row);
}

void BARTDispatcher::RunSampler(
  ...
  int cutpoint_grid_size, int random_seed
) {
