...
double GaussianUnivariateRegressionLeafModel::SplitLogMarginalLikelihood(GaussianUnivariateRegressionSuffStat& left_stat, GaussianUnivariateRegressionSuffStat& right_stat, double global_variance) {
  double left_log_ml = (
    -0.5*std::log(1 + tau_*(left_stat.sum_xxw/global_variance)) + ((tau_*(left_stat.sum_yxw/global_variance))/(2.0*(1+(left_stat.sum_xxw/global_variance))))
  );

  double right_log_ml = (
    -0.5*std::log(1 + tau_*(right_stat.sum_xxw/global_variance)) + ((tau_*(right_stat.sum_yxw/global_variance))/(2.0*(1+(right_stat.sum_xxw/global_variance))))
  );
...
double GaussianUnivariateRegressionLeafModel::NoSplitLogMarginalLikelihood(GaussianUnivariateRegressionSuffStat& suff_stat, double global_variance) {
  double log_ml = (
    -0.5*std::log(1 + tau_*(suff_stat.sum_xxw/global_variance)) + ((tau_*(suff_stat.sum_yxw/global_variance))/(2.0*(1+(suff_stat.sum_xxw/global_variance))))
  );
