...
void UnivariateResidual::LoadFromMemory(double* outcome_data_ptr, data_size_t num_row) {
  LoadData(outcome_data_ptr, num_row, residual);
}

void ConstantLeafForestDataset::LoadFromMemory(double* covariate_data_ptr, int num_covariate, data_size_t num_row, bool is_row_major) {
  LoadData(covariate_data_ptr, num_row, num_covariate, is_row_major, covariates);
}

void RegressionLeafForestDataset::LoadFromMemory(double* covariate_data_ptr, int num_covariate, double* basis_data_ptr, int num_basis, data_size_t num_row, bool is_row_major) {
  LoadData(covariate_data_ptr, num_row, num_covariate, is_row_major, covariates);
  LoadData(basis_data_ptr, num_row, num_basis, is_row_major, basis);
}

void ConstantRandomEffectsDataset::LoadFromMemory(std::vector<int32_t>& rfx_group_indices) {
