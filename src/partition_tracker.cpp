...
// ============================================================================
// ORDINAL AUXILIARY DATA METHODS
// ============================================================================
...
void ForestTracker::InitializeOrdinalAuxData(data_size_t num_observations, int n_levels) {
  ResizeOrdinalAuxData(num_observations, n_levels);
}
...
void ForestTracker::ResizeOrdinalAuxData(data_size_t num_observations, int n_levels) {
  // 4 types of ordinal auxiliary data: latent Z, forest predictions, cutpoints gamma, cumsum exp of gammas
  const int n_types = 4;
  ordinal_aux_data_vec_.resize(n_types);
  for (int i = 0; i < n_types; ++i) {
    if (i < 2) {
      // First two types (latent Z, forest predictions) are sized to num_observations
      ordinal_aux_data_vec_[i].assign(num_observations, 0.0);
    } else if (i == 2) {
      // Cutpoints gamma: size n_levels - 1
      ordinal_aux_data_vec_[i].assign(n_levels - 1, 0.0);
    } else if (i == 3) {
      // Cumsum exp of gammas: size n_levels
      ordinal_aux_data_vec_[i].assign(n_levels, 0.0);
    }
  }
}

// bool ForestTracker::IsValidOrdinalType(int type_idx) const {
//   return (type_idx >= 0 && type_idx < static_cast<int>(ordinal_aux_data_vec_.size()) && 
//           !ordinal_aux_data_vec_.empty());
// }

// bool ForestTracker::IsValidOrdinalIndex(int type_idx, data_size_t obs_idx) const {
//   if (!IsValidOrdinalType(type_idx)) {
//     return false;
//   }
//   return (obs_idx >= 0 && obs_idx < ordinal_aux_data_vec_[type_idx].size());
// }
