...
simplicity --- rather the intent is to provide a "research" 
interface to the C++ code that doesn't require modifying any C++. 
...
tau_preds <- predict_forest(forest_samples_ptr_tau, data_ptr_tau)*y_std
...
```{r}
plot(leaf_scale_samples_mu[1:num_warmstart], ylab="tau (prognostic)")
plot(leaf_scale_samples_tau[1:num_warmstart], ylab="tau (treatment)")
...
plot(rowMeans(tau_preds[,1:num_warmstart]/Z), tau_X, pch=16, cex=0.75, xlab = "pred", ylab = "actual", main = "treatment effect term");
abline(0,1,col="red",lty=2,lwd=2.5)
```
...
```{r}
plot(leaf_scale_samples_mu[(num_warmstart+1):num_samples], ylab="tau (prognostic)")
plot(leaf_scale_samples_tau[(num_warmstart+1):num_samples], ylab="tau (treatment)")
...
abline(0,1,col="red",lty=2,lwd=2.5)
plot(rowMeans(tau_preds[,(num_warmstart+1):num_samples]/Z), tau_X, pch=16, cex=0.75, xlab = "pred", ylab = "actual", main = "treatment effect term");
abline(0,1,col="red",lty=2,lwd=2.5)
```
