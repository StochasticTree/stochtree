...
TEST(Ensemble, Predict) {
  ...
  const char* params = "header=true label_column=5 treatment_column=-1 num_trees=2";
  ...
  StochTree::TreeEnsemble ensemble(config);

  // Set the first tree with a simple split rule
  auto* tree = ensemble.GetTree(0);
  tree->ExpandNode(0, 0, 0.5, true, 0., -5, 5, StochTree::Tree::kInvalidNodeId);
  tree = ensemble.GetTree(1);
  tree->ExpandNode(0, 1, 0.5, true, 0., -2.5, 2.5, StochTree::Tree::kInvalidNodeId);
