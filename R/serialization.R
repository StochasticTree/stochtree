...
CppJson <- R6::R6Class(
    ...
    public = list(
        ...
        add_random_effects = function(rfx_samples) {
            rfx_container_label <- json_add_rfx_container_cpp(
                self$json_ptr,
                rfx_samples$rfx_container_ptr
            )
            self$rfx_container_labels <- c(
                self$rfx_container_labels,
                rfx_container_label
            )
            rfx_mapper_label <- json_add_rfx_label_mapper_cpp(
                self$json_ptr,
                rfx_samples$label_mapper_ptr
            )
            self$rfx_mapper_labels <- c(
                self$rfx_mapper_labels,
                rfx_mapper_label
            )
            rfx_groupid_label <- json_add_rfx_groupids_cpp(
                self$json_ptr,
                rfx_samples$training_group_ids
            )
            self$rfx_groupid_labels <- c(
                self$rfx_groupid_labels,
                rfx_groupid_label
            )
        ...
        add_integer = function(field_name, field_value, subfolder_name = NULL) {
            if (is.null(subfolder_name)) {
                ...
                json_add_integer_subfolder_cpp(
                    self$json_ptr,
                    subfolder_name,
                    field_name,
                    field_value
        ...
        add_integer_vector = function(
            ...
            field_vector <- as.numeric(field_vector)
            if (is.null(subfolder_name)) {
                json_add_integer_vector_cpp(
                    self$json_ptr,
                    field_name,
                    field_vector
                )
            } else {
                json_add_integer_vector_subfolder_cpp(
                    self$json_ptr,
                    subfolder_name,
                    field_name,
                    field_vector
        ...
        add_string_vector = function(
            ...
            if (is.null(subfolder_name)) {
                ...
                json_add_string_vector_subfolder_cpp(
                    self$json_ptr,
                    subfolder_name,
                    field_name,
                    field_vector
        ...
        get_scalar = function(field_name, subfolder_name = NULL) {
            if (is.null(subfolder_name)) {
                ...
                stopifnot(json_contains_field_subfolder_cpp(
                    self$json_ptr,
                    subfolder_name,
                    field_name
                ))
                result <- json_extract_double_subfolder_cpp(
                    self$json_ptr,
                    subfolder_name,
                    field_name
                )
        ...
        get_integer = function(field_name, subfolder_name = NULL) {
            if (is.null(subfolder_name)) {
                ...
                result <- json_extract_integer_subfolder_cpp(
                    self$json_ptr,
                    subfolder_name,
                    field_name
                )
        ...
        get_boolean = function(field_name, subfolder_name = NULL) {
            if (is.null(subfolder_name)) {
                ...
                result <- json_extract_bool_subfolder_cpp(
                    self$json_ptr,
                    subfolder_name,
                    field_name
                )
        ...
        get_string = function(field_name, subfolder_name = NULL) {
            if (is.null(subfolder_name)) {
                ...
                result <- json_extract_string_subfolder_cpp(
                    self$json_ptr,
                    subfolder_name,
                    field_name
                )
        ...
        get_vector = function(field_name, subfolder_name = NULL) {
            if (is.null(subfolder_name)) {
                ...
                result <- json_extract_vector_subfolder_cpp(
                    self$json_ptr,
                    subfolder_name,
                    field_name
                )
        ...
        get_integer_vector = function(field_name, subfolder_name = NULL) {
            if (is.null(subfolder_name)) {
                stopifnot(json_contains_field_cpp(self$json_ptr, field_name))
                result <- json_extract_integer_vector_cpp(
                    self$json_ptr,
                    field_name
                )
            } else {
                ...
                result <- json_extract_integer_vector_subfolder_cpp(
                    self$json_ptr,
                    subfolder_name,
                    field_name
                )
        ...
        get_string_vector = function(field_name, subfolder_name = NULL) {
            if (is.null(subfolder_name)) {
                stopifnot(json_contains_field_cpp(self$json_ptr, field_name))
                result <- json_extract_string_vector_cpp(
                    self$json_ptr,
                    field_name
                )
            } else {
                ...
                result <- json_extract_string_vector_subfolder_cpp(
                    self$json_ptr,
                    subfolder_name,
                    field_name
                )
        ...
        get_numeric_list = function(field_name, key_names) {
            ...
            for (i in 1:length(key_names)) {
                ...
                output[[vec_name]] <- json_extract_vector_subfolder_cpp(
                    self$json_ptr,
                    field_name,
                    vec_name
                )
        ...
        get_string_list = function(field_name, key_names) {
            ...
            for (i in 1:length(key_names)) {
                vec_name <- key_names[i]
                output[[vec_name]] <- json_extract_string_vector_subfolder_cpp(
                    self$json_ptr,
                    field_name,
                    vec_name
                )
...
loadRandomEffectSamplesCombinedJsonString <- function(
    ...
    for (i in 1:length(json_string_list)) {
        ...
        if (i == 1) {
            ...
            output$append_from_json_string(
                json_string,
                json_rfx_container_label,
                json_rfx_mapper_label,
                json_rfx_groupids_label
