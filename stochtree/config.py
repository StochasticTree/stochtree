...
from .utils import _standardize_array_to_np, _check_is_int, _check_matrix_square
...
class ForestModelConfig:
    ...
    def __init__(self, num_trees = None, num_features = None, num_observations = None, 
                 feature_types = None, variable_weights = None, leaf_dimension = 1, 
                 alpha = 0.95, beta = 2.0, min_samples_leaf = 5, max_depth = -1, 
                 leaf_model_type = 1, leaf_model_scale = None, variance_forest_shape = 1.0, 
                 variance_forest_scale = 1.0, cutpoint_grid_size = 100) -> None:
        # Preprocess inputs and run some error checks
        if not feature_types:
            if not num_features:
                raise ValueError("Neither of `num_features` nor `feature_types` (a vector from which `num_features` can be inferred) was provided.", 
        ...
        if not variable_weights:
            warnings.warn("`variable_weights` not provided, will be assumed to be equal-weighted")
        ...
        if num_features != len(self.variable_weights):
            raise ValueError("`variable_weights` must have `num_features` total elements")
        ...
        if not leaf_model_scale:
            ...
            if isinstance(leaf_model_scale, np.ndarray):
                if not _check_matrix_square(leaf_model_scale):
                    raise ValueError("`leaf_model_scale` must be a square matrix if provided as a numpy array")
                leaf_model_scale_array = leaf_model_scale
            elif isinstance(leaf_model_scale, (int, float)):
                if leaf_model_scale <= 0:
                    raise ValueError("`leaf_model_scale` must be positive, if provided as scalar")
                leaf_model_scale_array = np.zeros((leaf_dimension, leaf_dimension), float)
                np.fill_diagonal(leaf_model_scale_array, leaf_model_scale)
            else:
                raise ValueError("`leaf_model_scale` must be a scalar value or a 2d numpy array with matching dimensions")
    ...
    def update_variable_weights(self, variable_weights: Union[list, np.ndarray]) -> None:
        """
    ...
    def update_leaf_model_scale(self, leaf_model_scale: Union[float, np.ndarray]) -> None:
        """
        ...
        if isinstance(leaf_model_scale, np.ndarray):
            ...
            if leaf_model_scale <= 0:
                raise ValueError("`leaf_model_scale` must be positive, if provided as scalar")
            leaf_model_scale_array = np.zeros((self.leaf_dimension, self.leaf_dimension), float)
            np.fill_diagonal(leaf_model_scale_array, leaf_model_scale)
        else:
            raise ValueError("`leaf_model_scale` must be a scalar value or a 2d numpy array with matching dimensions")
