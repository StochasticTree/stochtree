...
class RandomEffectsDataset:
    ...
    def add_group_labels(self, group_labels: np.array):
        ...
        if group_labels_.ndim > 1:
            raise ValueError("group_labels must be a one-dimensional numpy array of group indices")
    ...
    def add_basis(self, basis: np.array):
        ...
        if basis_.ndim != 2:
            raise ValueError("basis must be a one-or-two-dimensional numpy array of random effect bases")
        n, p = basis_.shape
    ...
    def add_variance_weights(self, variance_weights: np.array):
        ...
        if variance_weights_.ndim > 1:
            raise ValueError("variance_weights must be a one-dimensional numpy array of group indices")
...
class RandomEffectsContainer():
    ...
    def __init__(self, num_components: int, num_groups: int, rfx_tracker: RandomEffectsTracker) -> None:
        self.rfx_container_cpp = RandomEffectsContainerCpp(num_components, num_groups)
        self.rfx_label_mapper_cpp = RandomEffectsLabelMapperCpp(rfx_tracker.rfx_tracker_cpp)
    ...
    def predict(self, group_labels: np.array, basis: np.array) -> np.ndarray:
        ...
        return self.rfx_container_cpp.Predict(rfx_dataset.rfx_dataset_cpp, self.rfx_label_mapper_cpp)
...
class RandomEffectsModel:
    ...
    def sample(self, rfx_dataset: RandomEffectsDataset, residual: Residual, 
               rfx_tracker: RandomEffectsTracker, rfx_container: RandomEffectsContainer, 
               keep_sample: bool, global_variance: float, rng: RNG) -> None:
        """
        ...
        rng: RNG
            Object of type `RNG`
        """
        self.rfx_model_cpp.SampleRandomEffects(rfx_dataset.rfx_dataset_cpp, residual.residual_cpp, 
                                               rfx_tracker.rfx_tracker_cpp, rfx_container.rfx_container_cpp, 
                                               keep_sample, global_variance, rng.rng_cpp)
    ...
    def set_working_parameter(self, working_parameter: np.ndarray) -> None:
        ...
        working_parameter_ = np.squeeze(working_parameter) if working_parameter.ndim > 1 else working_parameter
        if working_parameter_.ndim != 1:
            raise ValueError("working_parameter must be a 1d numpy array with as many elements as bases in the random effects model")
    ...
    def set_group_parameters(self, group_parameters: np.ndarray) -> None:
        ...
        group_parameters_ = np.squeeze(group_parameters) if group_parameters.ndim > 2 else group_parameters
        if group_parameters_.ndim != 2:
            raise ValueError("group_parameters must be a 2d numpy array with as many rows as bases and as many columns as groups in the random effects model")
    ...
    def set_working_parameter_covariance(self, covariance: np.ndarray) -> None:
        ...
        if covariance_.ndim != 2:
            raise ValueError("covariance must be a 2d numpy array with as many rows and columns as bases in the random effects model")
