...
def calibrate_global_error_variance(X: np.array, y: np.array, nu: float = 3, q: float = 0.9, standardize: bool = True) -> float:
    ...
    if _is_model_overdetermined(lm_calibrator, n, mse, eps):
        ...
        warnings.warn("Default calibration method for global error variance failed; covariate dimension exceeds number of samples. "
                      "Initializing global error variance scale parameter based on the variance of the standardized outcome.", UserWarning)
    else:
        sigma2hat = mse
