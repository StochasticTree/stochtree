...
plot(rowMeans(xbcf_model$y_hat_test), y_test)
abline(0, 1)
cat(sqrt(mean((rowMeans(xbcf_model$y_hat_test) - y_test)^2)), "\n")
cat(
  mean(
    (apply(xbcf_model$y_hat_test, 1, quantile, probs = 0.05) <= y_test) &
      (apply(xbcf_model$y_hat_test, 1, quantile, probs = 0.95) >= y_test)
  ),
  "\n"
)
...
# Inspect the results
yhat_combined <- predict(combined_bcf, X = X_test)$y_hat
par(mfrow = c(1, 2))
...
cat(
  mean(
    (apply(bcf_model$y_hat_test, 1, quantile, probs = 0.05) <= y_test) &
      (apply(bcf_model$y_hat_test, 1, quantile, probs = 0.95) >= y_test)
  ),
  "\n"
)
