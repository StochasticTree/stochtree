...
# Load from source so the C++ fast path (bart_fit_cpp etc.) is available.
# devtools::load_all() compiles if needed; the installed CRAN/release package
# may not include the dispatch API yet.
if (requireNamespace("devtools", quietly = TRUE)) {
  ...
  library(stochtree)
  if (!exists("bart_fit_cpp", envir = asNamespace("stochtree"))) {
    warning(
      "The installed stochtree package does not expose bart_fit_cpp.\n",
      "Install devtools and run from the repo root so the fast path is loaded.\n",
      "Both columns will show the slow-path time."
    )
  }
}
  ...
  identity_basis = list(
    setup = function() {
      ...
      list(
        ...
        y_train = sp$y_train,
        X_test = sp$X_test,
        W_train = W[sp$train_inds, ],
        W_test = W[sp$test_inds, ]
      )
    },
    run = function(d) {
      invisible(bart(
        ...
        y_train = d$y_train,
        X_test = d$X_test,
        leaf_basis_train = d$W_train,
        leaf_basis_test = d$W_test,
        num_gfr = NUM_GFR,
        num_burnin = NUM_BURNIN,
        num_mcmc = NUM_MCMC,
...
selected <- if (length(args) > 0) {
  intersect(args, names(scenarios))
} else {
  names(scenarios)
}
