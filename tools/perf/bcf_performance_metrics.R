...
result_dgp1 <- foreach(i = 1:n_sim, .combine = rbind, .packages = c("stochtree")) %dopar% {
    set.seed(i)
    simulation_function(n, mu=mu1, tau=tau2, gfr_iter=20, burnin_iter=1000, 
                        mcmc_iter=1000, alpha=0.05, snr=2, sample_tau = F)
}

# Run the simulation for dgp 2
result_dgp2 <- foreach(i = 1:n_sim, .combine = rbind, .packages = c("stochtree")) %dopar% {
    set.seed(i)
    simulation_function(n, mu=mu2, tau=tau2, gfr_iter=20, burnin_iter=1000, 
                        mcmc_iter=1000, alpha=0.05, snr=2, sample_tau = F)
}
