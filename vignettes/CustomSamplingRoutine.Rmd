...
for (i in 1:num_warmstart) {
    ...
    global_var_samples[i+1] <- sampleGlobalErrorVarianceOneIteration(
...
if (num_gfr > 0){
    for (i in 1:num_gfr) {
        ...
        global_var_samples[i] <- sampleGlobalErrorVarianceOneIteration(
        ...
        # Sample variance parameters (if requested)
        global_var_samples[i] <- sampleGlobalErrorVarianceOneIteration(outcome, forest_dataset_tau, rng, nu, lambda)
        current_sigma2 <- global_var_samples[i]
    }
}
```
...
if (num_burnin + num_mcmc > 0) {
    for (i in (num_gfr+1):num_samples) {
        ...
        forest_model_mu$sample_one_iteration(
            ...
            cutpoint_grid_size, keep_forest = T, gfr = F
        )
        
        # Sample global variance parameter
        global_var_samples[i] <- sampleGlobalErrorVarianceOneIteration(outcome, forest_dataset_mu, rng, nu, lambda)
        current_sigma2 <- global_var_samples[i]
