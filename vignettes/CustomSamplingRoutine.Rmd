...
```{r}
plot(
  rowMeans(forest_preds[, (num_warmstart + 1):num_samples]),
  y,
  pch = 16,
  cex = 0.75,
  xlab = "pred",
...
lm_preds <- (sapply(1:num_samples, function(x) W[, 1] * beta_samples[x + 1])) *
  y_std
...
forest_model_config_mu <- createForestModelConfig(
  ...
  num_features = ncol(X_mu),
  num_observations = nrow(X_mu),
...
forest_model_config_tau <- createForestModelConfig(
  ...
  num_features = ncol(X_tau),
  num_observations = nrow(X_tau),
...
plot(
  rowMeans(mu_hat[, 1:num_gfr]),
  mu_x,
  pch = 16,
  cex = 0.75,
  xlab = "pred",
  ylab = "actual",
  main = "prognostic term"
)
abline(0, 1, col = "red", lty = 2, lwd = 2.5)
plot(
  rowMeans(tau_hat[, 1:num_gfr]),
  tau_x,
  pch = 16,
  cex = 0.75,
  xlab = "pred",
  ylab = "actual",
  main = "treatment effect term"
)
abline(0, 1, col = "red", lty = 2, lwd = 2.5)
...
plot(
  rowMeans(mu_hat[, (num_gfr + 1):num_samples]),
  mu_x,
  pch = 16,
  cex = 0.75,
  xlab = "pred",
...
abline(0, 1, col = "red", lty = 2, lwd = 2.5)
plot(
  rowMeans(tau_hat[, (num_gfr + 1):num_samples]),
  tau_x,
  pch = 16,
  cex = 0.75,
  xlab = "pred",
...
plot(
  b_0_samples,
  col = "blue",
  ylab = "Coding parameter draws",
  ylim = c(
    min(min(b_0_samples), min(b_1_samples)),
    max(max(b_0_samples), max(b_1_samples))
  )
)
