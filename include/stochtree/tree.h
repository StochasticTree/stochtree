...
inline bool SplitTrueCategorical(double fvalue, std::vector<std::uint32_t> const& category_list) {
  ...
  auto max_representable_int = std::min(static_cast<double>(std::numeric_limits<std::uint32_t>::max()),
  ...
  if (fvalue < 0 || std::fabs(fvalue) > max_representable_int) {
    ...
    category_matched = (std::find(category_list.begin(), category_list.end(), category_value) != category_list.end());
  }
...
class TreeSplit {
 public:
  ...
  bool SplitTrue(double fvalue) {
    if (numeric_)
      return SplitTrueNumeric(fvalue, split_value_);
    else
      return SplitTrueCategorical(fvalue, split_categories_);
  }
